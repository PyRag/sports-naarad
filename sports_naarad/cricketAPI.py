...
class Cricket(object):
    ...
    def live_score(self, type_return='string'):
        ...
        response = requests.get(
            'http://www.cricbuzz.com/live-scores', stream=True, proxies=proxy_dict)
    ...
    def list_matches(self, type_return='string'):
        response = requests.get(
            'https://cricket.yahoo.com/matches/schedule', stream=True, proxies=proxy_dict)
        soup = bs4.BeautifulSoup(response.text, parser)
        head_list = soup.findAll("em", {"class": "ycric-table-heading"})
        invited_team_list = soup.findAll(
            "div", {"class": "ycric-table-sub-heading"})
        ...
        for i in range(cnt):
            if nos[i] == "1":
                ...
                matches[header] = []
                heading = heading + 1
            matches[header].append((team_list[i].lstrip(), tour_dates_list[
                                   i].text.lstrip(), venue[i].lstrip(), result[i].lstrip()))
        if type_return == 'dict':
            return matches
        return str(matches)
    ...
    def news(self, type_return='string'):
        ...
        for all_news in news:
            if str(all_news.get("title")) != "More Photos" and str(all_news.get("title")) != "None":
                news_dict[all_news.get("title")] = base_url + \
                    all_news.get("href")
        if type_return == 'dict':
            return news_dict
